...
assert.deepEqual(getSignalRouteCommandLabels('S608'), ['Route R608_600', 'Route R608_602', 'Route R608_803'])
assert.deepEqual(getSignalRouteCommandLabels('S709'), [])

assert.deepEqual(
  getSignalRouteCommandStateSegmentIds('S700', 'Route R700_608'),
  ['route-r700-608-command'],
)
assert.deepEqual(
...
assert.equal(isSignalRouteCommandState(routeState('route-r700-608-command', 'SET')), true)
assert.equal(isSignalRouteCommandState(routeState('route-r700-608-command', 'UNSET')), false)
assert.equal(isSignalRouteCommandState(routeState('route-r700-608-command', 'SET', 0)), false)
...
{
  const map = lineMap({
    'route-r700-608-command': routeState('route-r700-608-command', 'SET'),
  })

  assert.deepEqual(getSignalRouteSetLabels('S700', map), ['Route R700_608'])
